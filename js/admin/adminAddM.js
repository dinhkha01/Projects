      ...
      <div class="_right">
        <div class="the-content-new-car">
          ...
          <div class="m-132-button-bar">
            ...
            <button class="buttonD" onclick="deleteProduct('${carMer[i].id}')">Xóa>></button>
            <button class="buttonE" onclick="editProduct('${carMer[i].id}')">Chỉnh sửa>></button>
...
function deleteProduct(id) {
  ...
  const index = carMer.findIndex((product) => product.id === id);

  if (index !== -1) {
    // Xóa sản phẩm khỏi mảng carBMW
    carMer.splice(index, 1);
...
function renderProductItem(id) {
  ...
  if (isLoggedIn) {
    ...
    window.location.href = "/pages/admin/adminHienthiM.html";
  } else {
